...
import LocationIcon from './icons/location-pin-svgrepo-com.svg';
...
function Weather() {
  ...
  useEffect(()=>{
    if(navigator.geolocation){
      navigator.geolocation.getCurrentPosition(pos=>{
        ...
        const apiUrl = `https://api.openweathermap.org/data/2.5/weather?lat=${pos.coords.latitude.toFixed(1)}&lon=${pos.coords.longitude.toFixed(1)}&appid=${apiKey}`;
  ...
  return (
      <div className="area-weather">
        ...
        <img className='weather-icon' src={`https://openweathermap.org/img/wn/${(weatherIconId)?weatherIconId:"02d"}@2x.png`} alt='weather'/>
          {usersTemp}°C
        </span>
      </div>
  );
